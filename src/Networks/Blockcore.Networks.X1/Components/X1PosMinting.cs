...
namespace Blockcore.Networks.X1.Components
{
    ...
    /// </para>
    /// </remarks>
    public class X1PosMinting : IPosMinting
    {
        /// <summary>
        ...
        private void CoinstakeWorker(CoinstakeWorkerContext context, ChainedHeader chainTip, Block block, long minimalAllowedTime, long searchInterval)
        {
            ...
            foreach (UtxoStakeDescription utxoStakeInfo in orderedUtxoStakeDescriptions)
            {
                ...
                for (uint n = 0; n < searchInterval; n++)
                {
                    ...
                    try
                    {
                        OutPoint prevoutStake = utxoStakeInfo.OutPoint;// new OutPoint(utxoStakeInfo.UtxoSet.TransactionId, utxoStakeInfo.OutPoint.N);
                        ...
                        if (context.Result.SetKernelFoundIndex(context.Index))
                        {
                            ...
                            Wallet wallet = this.walletManager.GetWalletByName(utxoStakeInfo.Secret.WalletName);
        ...
        private bool SignTransactionInput(UtxoStakeDescription input, Transaction transaction)
        {
            ...
            try
            {
                TransactionBuilder transactionBuilder = new TransactionBuilder(this.network)
                    .AddKeys(input.Key);
